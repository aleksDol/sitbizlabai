  ...
  const result = await requestOpenAiText({
    messages: [
      ...
      },
      {
        role: "system",
        content:
          "Не используй markdown: не пиши ###, ##, #, не используй markdown-заголовки и символы разметки. Форматируй обычным текстом с разделением на блоки и эмодзи. Финальный ответ выдай строго в структуре: 📊 Оценка ситуации (2-4 предложения), ⚠️ Что мешает росту (до 3 пунктов), 🚀 Что стоит внедрить (для каждого пункта: Проблема, Решение, Результат), 📈 Если коротко (Сейчас: ... После: ...), 🛠 Что мы можем сделать для вас (краткий список работ), 👉 Финал: 💡 Хотите внедрить это у себя? Оставьте контакты — покажем, как реализовать под ваш бизнес. В блоке 'Что мы можем сделать для вас' используй только релевантные пункты: лендинг/сайт, Telegram-бот при необходимости, CRM/админка и аналитика при нескольких источниках/потере заявок, связка в единую систему заявок и повторных касаний."
      },
      {
        role: "user",
