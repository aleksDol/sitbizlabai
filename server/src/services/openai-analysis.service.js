...
function normalizePreviewPayload(preview) {
  if (!preview || typeof preview !== "object") {
    return null;
  }

  const rawProblems = Array.isArray(preview.problems) ? preview.problems : [];
  ...
  const recommendationTitleRaw =
    typeof preview?.recommendation?.title === "string" ? preview.recommendation.title : "Что можно улучшить";
  const recommendationTitle = sanitizePreviewText(recommendationTitleRaw) || "Что можно улучшить";

  if (problems.length !== 2 || !recommendationText) {
    return null;
  }

  if (problems.some((item) => item.text.split(/\s+/).length < 8)) {
    return null;
  }

  if (recommendationText.split(/\s+/).length < 6) {
    return null;
  }
...
function extractAnalysisPayload(rawText) {
  ...
  const parseCandidate = (candidate) => {
    try {
      ...
      const analysisText =
        typeof data.analysisText === "string" && data.analysisText.trim() ? data.analysisText.trim() : "";
      if (!analysisText) {
        return null;
      }
      return {
        analysisText,
        preview: normalizePreviewPayload(data.preview)
      };
    } catch {
      return null;
    }
...
export async function fetchOpenAiAnalysis(siteData) {
  ...
  const parsed = extractAnalysisPayload(result.text || "");
  const analysisText = parsed?.analysisText || result.text || OPENAI_ANALYSIS_FALLBACK;

  return {
    ...result,
    text: applyAnalysisQualityChecks(analysisText),
